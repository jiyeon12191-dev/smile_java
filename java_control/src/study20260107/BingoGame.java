...
public class BingoGame {
	...
	public static void main(String[] args) {
		...
		// %d - 정수 , %f - 실수 , %c-문자, %s-문자열, %0-8진수, %x-16진수
		while(true ) {
	   System.out.println("┌─────┬─────┬─────┬─────┬─────┐");
		for(int i=0; i<5; i++) {	
			for(int k=0; k<5; k++) 
		...
		// 빙고를 위해 숫자 입력 만들기
			System.out.print(" 입력 : ");
			int user = sc.nextInt();
			for(int k=0; k< bingo.length; k++) { // 배열 에서 내가 입력한 숫자 찾기 
				if( bingo[k] == user) { // 입력한 값이 배열에 있다면
					bingo[k] = 0; // 화면에 체크 표시줄 위에 0으로 변경
				}
				...
				if(row==5) end++; // 해당 줄에 0이 5개라면 row는 5가된다. 1빙고완성
				if(col==5) end++; // 세로방향의 열 , 0이 5개라면 col은 5, 1빙고완성
