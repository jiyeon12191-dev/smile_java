...
window.onload=function(){
    ...
    const  필수입력 = document.getElsementsByClassName("essential");

    for(var input of 필수입력){
        input.addEventListener("key up", joinCheck);

    }
    
    // querySelector - 자바스크립트로 태그를가져오는방법 (동적)
...
function joinCheck(){
    ...
    if( count == 5){// 필수 입력 모두 했다.
        if(pw.value != pw2.value){// 비밀번호와 비밀번호 확인이 다른값
